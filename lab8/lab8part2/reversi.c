...
bool checkCorners(const char board[][26], int n, int row, int col,
      ...
      for (int deltaCol = -1; deltaCol <= 1; deltaCol++) {
        if (positionInBounds(n, row + deltaRow, col + deltaCol)) {
          if (board[row + deltaRow][col + deltaCol] == 'B') {
            whiteCount += 7;  // arbitrary number, change later
          }
        } else {
          continue;
        }
      ...
      for (int deltaCol = -1; deltaCol <= 1; deltaCol++) {
        if (positionInBounds(n, row + deltaRow, col + deltaCol)) {
          if (board[row + deltaRow][col + deltaCol] == 'W') {
            blackCount += 7;  // arbitrary number, change later
          }
        } else {
          continue;
        }
...
int makeMove(const char board[][26], int n, char turn, int *row, int *col) {
  ...
  checkCorners(board, n, 0, 0, &blackCount, &whiteCount);
  checkCorners(board, n, 0, n, &blackCount, &whiteCount);
  checkCorners(board, n, n, 0, &blackCount, &whiteCount);
  checkCorners(board, n, n, n, &blackCount, &whiteCount);
...
int main(void) {
  ...
  while (!gameOver(board, n)) {
    if (turn == computer) {
      int row, col;
      makeMove(board, n, computer, &row, &col);
    } else {
      char row, col;
